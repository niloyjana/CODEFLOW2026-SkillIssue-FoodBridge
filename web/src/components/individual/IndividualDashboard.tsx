...
export const IndividualDashboard: React.FC<IndividualDashboardProps> = ({ posts, loading, onDeletePost }) => {
  const { user } = useAuth();
  ...
  // Filter posts created by the current individual volunteer (excl. deleted)
          ...
          myPosts.map((post) =>
            React.createElement(
              ...
              post.status === 'claimed' && React.createElement(
                ...
                post.claimedByName ? `Claimed by ${post.claimedByName} volunteer/shelter.` : 'Claimed by a volunteer and scheduled for pickup.'
              ),
